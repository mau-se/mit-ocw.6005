...
public class AlphabetGenerator {
    ...
    public static char[] generateFrequencyAlphabet(int base,
    	...
    	for(int i = 0; i < (allChars.length()); i++) {
    		...
    		numberOfCharacters.put(key , (count == null) ? 1: count + 1);
    	}
    	for(Integer i: (numberOfCharacters.values())) { 
    		total = total + i;
    	}
    	for(Character c: (numberOfCharacters.keySet())) {
    		double pdf = (double)(numberOfCharacters.get(c))/total;
    		cdf = cdf + pdf;
    		int amount = (int)(cdf*base);
    		numberOfCharacters.put(c, amount);
    		
    		for(int i = numberOfCharacters.get(c)-1;
    				i >= index; i--) {
    			mapOfChars[i] = c;
    		}
    		
    		index = numberOfCharacters.get(c);
    	}
